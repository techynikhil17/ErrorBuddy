...
function detectRuntime(raw: string): DetectedRuntime {
...
function runtimeToErrorType(runtime: DetectedRuntime): string {
  switch (runtime) {
...
const STACK_FRAME_RE =
  /^\s*(at\s+|goroutine\s+\d+|File\s+"|from\s+\/|#\d+\s+0x)/i;

function extractSignalLine(lines: string[], runtime: DetectedRuntime): string {
  ...
  if (runtime === "python") {
    for (let i = lines.length - 1; i >= 0; i--) {
      ...
      if (t && !STACK_FRAME_RE.test(t)) return t;
    }
  }
...
function extractPortFromText(text: string): number | undefined {
...
function extractModuleFromText(text: string): string | undefined {
