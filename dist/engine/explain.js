...
function explainError(error, classified = (0, classify_1.classifyError)(error)) {
    const shared = {
    ...
    switch (classified.category) {
        ...
        case "UNDEFINED_ERROR": {
            ...
            const undefinedEntity = (0, entityExtractor_1.extractEntity)(error.message);
            const undefinedEntityType = undefinedEntity
                ? (0, entityExtractor_1.detectEntityType)(error.message)
                : "resource";
            return {
        ...
        case "FILE_NOT_FOUND": {
            const fileEntity = classified.details.moduleName ??
                (0, entityExtractor_1.extractEntity)(error.message);
            const fileEntityType = fileEntity
                ? (0, entityExtractor_1.detectEntityType)(error.message.includes("directory") ? "file" : error.message)
                : "file";
            return {
        ...
        case "MODULE_NOT_FOUND": {
            const moduleEntity = classified.details.moduleName ??
                (0, entityExtractor_1.extractEntity)(error.message);
            return {
        ...
        case "HTTP_ERROR": {
            const statusCode = classified.details.statusCode ?? (0, classify_1.extractStatusCode)(error.message);
            const statusCategory = classified.details.statusCategory ?? (0, classify_1.classifyHttpStatus)(statusCode ?? null);
            return {
                title: statusCode ? `API Error (${statusCode})` : "API Error",
                what: statusCategory === "SERVER_ERROR"
        ...
        case "CONVEX_FUNCTION_NOT_FOUND": {
            const keyLine = (0, signatures_1.extractKeyMessage)(error.message);
            const convexEntity = keyLine ? (0, entityExtractor_1.extractEntity)(keyLine) : (0, entityExtractor_1.extractEntity)(error.message);
            return {
        ...
        case "PYTHON_ERROR": {
            const pyEntity = (0, entityExtractor_1.extractEntity)(error.message);
...
function buildHttpCause(statusCategory) {
