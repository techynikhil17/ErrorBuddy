...
exports.ERROR_SIGNATURES = void 0;
exports.extractStatusCode = extractStatusCode;
exports.classifyHttpStatus = classifyHttpStatus;
exports.isHttpError = isHttpError;
exports.extractKeyMessage = extractKeyMessage;
exports.ERROR_SIGNATURES = [
    {
        id: "commander-config",
    ...
    },
    {
        ...
        confidence: 0.93,
        match: (error) => /unexpected token .*json|unexpected end of json input|json\.parse|json at position/i.test(buildHaystack(error)),
        extract: (error) => ({
    ...
    },
    {
        ...
        confidence: 0.92,
        match: (error) => /unhandledpromiserejection|unhandled rejection|promise rejected|rejected promise|a promise was rejected/i.test(buildHaystack(error)),
        extract: (error) => ({
    ...
    },
    {
        ...
        category: "PRISMA_ERROR",
        confidence: 0.96,
        match: (error) => /PrismaClientInitializationError|Can't reach database server/i.test(buildHaystack(error)),
        extract: (error) => ({
    ...
    // ─── Docker ───────────────────────────────────────────────────────────────
    {
        ...
        confidence: 0.96,
        match: (error) => /Error response from daemon:|Cannot connect to the Docker daemon/i.test(buildHaystack(error)),
    ...
    // ─── Python ───────────────────────────────────────────────────────────────
    {
        ...
        confidence: 0.95,
        match: (error) => /Traceback \(most recent call last\):|File ".+?\.py", line/i.test(buildHaystack(error)),
    ...
    // ─── System resources ─────────────────────────────────────────────────────
    {
        ...
        confidence: 0.97,
        match: (error) => /ENOMEM|out of memory|JavaScript heap out of memory/i.test(buildHaystack(error)),
    ...
    // ─── Database (generic) ───────────────────────────────────────────────────
    {
        ...
        confidence: 0.96,
        match: (error) => /FATAL: password authentication failed|FATAL: role .+ does not exist|could not connect to server/i.test(buildHaystack(error)),
    ...
    },
    {
        ...
        category: "BUILD_ERROR",
        confidence: 0.93,
        match: (error) => /Module build failed|ModuleNotFoundError/i.test(buildHaystack(error)) &&
 ...
 */
function extractKeyMessage(msg) {
